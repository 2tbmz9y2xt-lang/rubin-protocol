...
External mixed_client_go_submit_rust_accept report revalidation is intentionally unsupported;
same-run tx proof validation is internal to --go-submit-rust-accept producer execution.
EOF
}
...
check_report() { local report="${1:-}" mode="${2:-offline}" expected_mode="${3:-auto}"
  # producer-tx is intentionally not exposed as a public CLI mode. It is only
  # used for the just-generated report after this process submitted and observed
  # the transaction through the Go-submit -> Rust-accept path.
...
if tx_mode and expected_mode != "producer-tx":
    if live:
        fail("public tx-path check-report-live is unsupported; same-run producer evidence is required")
...
legacy_schema_purpose = (
    "schema-valid legacy artifact only; not the Go-submit -> Rust-accept tx-path report verdict"
    if tx_mode
    else "schema-valid legacy artifact only; not the mixed-client mesh report verdict"
)
report = {
...
# producer-tx is same-run-only: the public --check-report* surface never uses it.
