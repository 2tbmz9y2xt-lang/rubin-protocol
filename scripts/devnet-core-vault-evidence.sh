...
SUBMIT_JSON=""; SUBMITTED_TXID=""; MINE_JSON=""; MINED_HEIGHT=""; MINED_HASH=""; BLOCK_JSON=""
PHASE="init"
...
write_report() {
  ...
  python3 - "${REPORT_JSON}" "${status}" "${PHASE}" "${reason}" "${FIXTURE_PATH}" "${VECTOR_ID}" \
    ...
    "${SUBMIT_JSON}" "${MINE_JSON}" "${BLOCK_JSON}" "${DEVNET_CHAIN_ID}" "${SUBMITTED_TXID}" "${MINED_HEIGHT}" "${MINED_HASH}" <<'PY' || true
import json, sys
(path,status,phase,reason,fixture,vector,root,pid,rpc,datadir,identity,submit,mine,block,chain_id,txid,height,block_hash)=sys.argv[1:19]
def load(raw):
...
[[ -f "${FIXTURE_PATH}" ]] || fail FAIL_INPUT "canonical devnet CORE_VAULT fixture missing: ${FIXTURE_PATH}"
write_seed_go; mkdir -p "${NODE_DIR}" || fail FAIL_LOCAL_HARNESS "failed to create node datadir"
PHASE="seed_fixture_context"
TX_HEX="$({ "${DEV_ENV}" -- go -C "${GO_MODULE_ROOT}" run "${SEED_GO}" --datadir "${NODE_DIR}" --fixture "${FIXTURE_PATH}" --vector-id "${VECTOR_ID}" --chain-id "${DEVNET_CHAIN_ID}"; } 2>&1)" || fail FAIL_INPUT "fixture validation/seed failed: ${TX_HEX}"
PHASE="build"
...
printf '%s' "${CHAIN_IDENTITY_JSON}" | python3 -c 'import json,sys; d=json.load(sys.stdin); want=sys.argv[1]; sys.exit(0 if d.get("chain_id_hex")==want else 1)' "${DEVNET_CHAIN_ID}" || fail FAIL_LOCAL_HARNESS "live node chain_id mismatch: ${CHAIN_IDENTITY_JSON}"
