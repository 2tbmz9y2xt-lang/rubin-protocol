...
def write_atomic(path: Path, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
        raise
def path_field_targets(value: Any, base: Path, depth: int = 0, prefix: str = "") -> list[Path]:
    if depth > 200:
        raise ValueError("path_field_depth_exceeded")
    out: list[Path] = []
    if isinstance(value, dict):
        for key, item in value.items():
            dotted = f"{prefix}.{key}" if prefix else key
            if isinstance(item, str) and (key in PATH_FIELD_NAMES or key.endswith("_path") or dotted in PATH_FIELD_DOTTED_NAMES):
                raw = Path(os.path.expanduser(item)); out.append(Path(os.path.realpath(raw if raw.is_absolute() else base / raw)))  # noqa: E702
            else:
                out.extend(path_field_targets(item, base, depth + 1, dotted))
    elif isinstance(value, list):
        for item in value:
            out.extend(path_field_targets(item, base, depth + 1, prefix))
    return out
def output_overwrites_input(args: argparse.Namespace, out_path: Path) -> bool:
    for key, value in vars(args).items():
        ...
        if isinstance(data, dict):
            try:
                if out_path in path_field_targets(data, source_path.parent):
                    return True
            except ValueError:
                return True
    return False
