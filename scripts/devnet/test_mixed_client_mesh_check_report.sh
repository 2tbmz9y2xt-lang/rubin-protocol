...
PARTITION_DISCONNECTED_GO_FORK_REPORT="$(sed -n '74p' "${REPORT_LIST}")"
PARTITION_SAME_FIRST_RUST_FORK_REPORT="$(sed -n '75p' "${REPORT_LIST}")"
PARTITION_SKIP_HEIGHT_RUST_WINNER_REPORT="$(sed -n '76p' "${REPORT_LIST}")"
PARTITION_FORGED_METRICS_REPORT="$(sed -n '77p' "${REPORT_LIST}")"
PARTITION_PEER_SIDECAR_BOOL_COUNT_REPORT="$(sed -n '78p' "${REPORT_LIST}")"
PARTITION_PEER_SIDECAR_EXTRA_FIELD_REPORT="$(sed -n '79p' "${REPORT_LIST}")"
PARTITION_RUST_TIP_FLOAT_HEIGHT_REPORT="$(sed -n '80p' "${REPORT_LIST}")"
PARTITION_GO_TIP_STRING_IBD_REPORT="$(sed -n '81p' "${REPORT_LIST}")"
PARTITION_RUST_MINE_STRING_NONCE_REPORT="$(sed -n '82p' "${REPORT_LIST}")"
PARTITION_RUST_MINE_STRING_TIMESTAMP_REPORT="$(sed -n '83p' "${REPORT_LIST}")"
PARTITION_METRICS_TIMESTAMP_REPORT="$(sed -n '84p' "${REPORT_LIST}")"
PARTITION_METRICS_JSON_REPORT="$(sed -n '85p' "${REPORT_LIST}")"
