...
assert_block_txids() { RUBIN_OPENSSL_SKIP_FIPS_GUARD=1 "${DEV_ENV}" -- go -C "${GO_MODULE_ROOT}" run "${DATXGEN_GO}" blockcheck "$@"; }
p2p_addr_for_pid() {
  python3 - "$1" "$2" <<'PY'
...
func main() {
	if len(os.Args) == 7 && os.Args[1] == "blockcheck" { blockCheck(os.Args[2:]); return }
	if len(os.Args) != 3 { panic("usage: da-txgen <datadir> <from-key-file>") }
	txgen(os.Args[1], os.Args[2])
}
func txgen(datadir, keyFile string) {
	derHex, err := os.ReadFile(keyFile); if err != nil { panic(err) }
	...
	defer key.Close()
	st, err := node.LoadChainState(node.ChainStatePath(node.NormalizeDataDir(datadir))); if err != nil { panic(err) }
...
func blockCheck(args []string) {
	die := func(msg string) { panic(msg) }; raw, err := hex.DecodeString(strings.TrimSpace(args[0])); if err != nil { panic(err) }; pb, err := consensus.ParseBlockBytes(raw); if err != nil { panic(err) }; gotHash, err := consensus.BlockHash(pb.HeaderBytes); if err != nil { panic(err) }; wantCount, err := strconv.ParseUint(args[2], 10, 64); if err != nil { panic(err) }
	if hex.EncodeToString(gotHash[:]) != strings.ToLower(args[1]) { die("parsed block hash mismatch") }; if pb.TxCount != wantCount { die("parsed block tx_count mismatch") }
	seen := map[string]bool{}; for _, txid := range pb.Txids { seen[hex.EncodeToString(txid[:])] = true }
	for _, txid := range strings.Split(args[3], ",") { txid = strings.ToLower(strings.TrimSpace(txid)); if txid != "" && txid != "-" && !seen[txid] { die("missing txid " + txid) } }; for _, txid := range strings.Split(args[4], ",") { txid = strings.ToLower(strings.TrimSpace(txid)); if txid != "" && txid != "-" && seen[txid] { die("unexpected txid " + txid) } }
}
