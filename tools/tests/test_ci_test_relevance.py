...
def runner(payload=b"M\0docs/readme.md\0", *, verify_code=0, diff_code=0):
    def run(command, **_kwargs):
        code = verify_code if command[1] == "rev-parse" else diff_code
        output = b"base\n" if command[1] == "rev-parse" else payload
...
class TestRelevanceTests(unittest.TestCase):
    def decision(self, payload):
        return subject.decide("pull_request", "a" * 40, runner(payload))
    ...
    def test_malformed_empty_unknown_and_invalid_records_run(self):
        payloads = (b"", b"\0", b"M\0tools/a.py", b"R\0a\0b\0", b"R101\0a\0b\0", b"Cabc\0a\0b\0", b"X\0a\0", b"M\0\0", b"M\0../a\0", b"M\0/a\0", b"M\0\xff\0")
    ...
    def test_events_and_git_errors_run(self):
        ...
        self.assertTrue(subject.decide("pull_request", "a" * 40, runner(verify_code=1)).run_test)
        self.assertTrue(subject.decide("pull_request", "a" * 40, runner(diff_code=1)).run_test)
        def broken(*_args, **_kwargs):
            raise RuntimeError("boom")
        with self.assertRaises(RuntimeError):
            subject.decide("pull_request", "a" * 40, broken)
    ...
    def test_diff_command_enables_nul_rename_and_copy_detection(self):
        ...
        subject.decide("pull_request", base_sha, record)
        self.assertEqual(commands[0], ["git", "rev-parse", "--verify", f"{base_sha}^{{commit}}"])
        self.assertEqual(commands[1][:4], ["git", "diff", "--name-status", "-z"])
        self.assertEqual(commands[1][-1], f"{base_sha}..HEAD")
