...
class TestRelevanceTests(unittest.TestCase):
    ...
    def test_workflow_keeps_json_unconditional_and_gates_other_steps(self):
        ...
        self.assertEqual(steps[18].splitlines(), ["name: Go tests", f"        if: {gate}", "        run: |", "          cd clients/go", "          go test -v ./node/p2p -run '^TestCanonicalBlockRelayFixtureRows$' -count=1", "          go test ./..."])
        for step in steps[6:]:
