...
class EdgePackCheckerTests(unittest.TestCase):
    ...
    def test_runtime_evidence_rejects_go_raw_string_test_signature(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            go_test.parent.mkdir(parents=True, exist_ok=True)
            go_test.write_text(
                "package node\n\nconst fixture = `func TestRuntimeReorgCoverage(t *testing.T) {}`\n",
                encoding="utf-8",
            )
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
    ...
    def test_runtime_evidence_rejects_rust_raw_string_test_signature(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_cfg_disabled_rust_parent_module(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_macro_body_test(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_cfg_attr_ignored_rust_test(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
