...
def make_repo(
    ...
    *,
    ...
    rust_lib = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "lib.rs"
    rust_lib.parent.mkdir(parents=True, exist_ok=True)
    rust_lib.write_text("pub mod sync_reorg;\n", encoding="utf-8")
...
class EdgePackCheckerTests(unittest.TestCase):
    ...
    def test_runtime_evidence_rejects_go_test_with_return_value(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            go_test.parent.mkdir(parents=True, exist_ok=True)
            go_test.write_text(
                'package node\n\nimport "testing"\n\nfunc TestRuntimeReorgCoverage(t *testing.T) int { return 0 }\n',
                encoding="utf-8",
            )
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
    ...
    def test_runtime_evidence_rejects_go_test_without_testing_import(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_commented_go_testing_import(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_allows_go_testing_import_alias(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            go_test.parent.mkdir(parents=True, exist_ok=True)
            go_test.write_text(
                'package node\n\nimport tt "testing"\n\nfunc TestRuntimeReorgCoverage(t *tt.T) {}\n',
                encoding="utf-8",
            )
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stdout(captured):
    ...
    def test_runtime_evidence_rejects_go_build_tagged_test_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_go_ignored_file_name(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
                root,
                {"clients/go/node/_sync_reorg_test.go": ["TestRuntimeReorgCoverage"]},
            )
            go_test = root / "clients" / "go" / "node" / "_sync_reorg_test.go"
            go_test.parent.mkdir(parents=True, exist_ok=True)
            go_test.write_text(
                'package node\n\nimport "testing"\n\nfunc TestRuntimeReorgCoverage(t *testing.T) {}\n',
                encoding="utf-8",
            ...
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
        self.assertIn("unreachable runtime evidence source", captured.getvalue())

    def test_runtime_evidence_rejects_go_symlink_test_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            target = root / "outside_go_test.go"
            target.write_text(
                'package node\n\nimport "testing"\n\nfunc TestRuntimeReorgCoverage(t *testing.T) {}\n',
                encoding="utf-8",
            )
            ...
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
        self.assertIn("must not be a symlink", captured.getvalue())

    def test_runtime_evidence_rejects_go_platform_specific_test_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
                root,
                {"clients/go/node/sync_reorg_linux_test.go": ["TestRuntimeReorgCoverage"]},
            )
            go_test = root / "clients" / "go" / "node" / "sync_reorg_linux_test.go"
            go_test.parent.mkdir(parents=True, exist_ok=True)
            go_test.write_text(
                'package node\n\nimport "testing"\n\nfunc TestRuntimeReorgCoverage(t *testing.T) {}\n',
                encoding="utf-8",
            ...
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
        self.assertIn("platform-specific Go suffix", captured.getvalue())
    ...
    def test_runtime_evidence_allows_multiline_rust_attribute_before_test_function(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_cfg_disabled_rust_test(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_test_in_cfg_disabled_module(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_test_in_split_brace_cfg_disabled_module(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_test_in_cfg_disabled_function_container(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_test_inside_enabled_function_container(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_test_in_unlinked_module_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
                root,
                {"clients/rust/crates/rubin-node/src/unlinked_reorg.rs": ["runtime_reorg_coverage"]},
            )
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "unlinked_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_runtime_evidence_rejects_rust_symlink_source_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            target = root / "outside_sync_reorg.rs"
            target.write_text("#[test]\nfn runtime_reorg_coverage() {}\n", encoding="utf-8")
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "sync_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            try:
                rust_test.symlink_to(target)
            except OSError as exc:
                self.skipTest(f"symlink unavailable: {exc}")
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
    ...
    def test_runtime_evidence_rejects_rust_symlink_module_chain_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            lib_rs = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "lib.rs"
            lib_rs.write_text("pub mod reorg;\n", encoding="utf-8")
            module_target = root / "outside_reorg.rs"
            module_target.write_text("pub mod sync_reorg;\n", encoding="utf-8")
            module_link = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "reorg.rs"
            try:
                module_link.symlink_to(module_target)
            except OSError as exc:
                self.skipTest(f"symlink unavailable: {exc}")
            add_runtime_evidence(
                root,
                {"clients/rust/crates/rubin-node/src/reorg/sync_reorg.rs": ["runtime_reorg_coverage"]},
            )
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "reorg" / "sync_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text("#[test]\nfn runtime_reorg_coverage() {}\n", encoding="utf-8")
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
    ...
    def test_runtime_evidence_rejects_rust_test_in_cfg_disabled_external_module(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            lib_rs = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "lib.rs"
            lib_rs.write_text("#[cfg(FALSE)]\npub mod sync_reorg;\n", encoding="utf-8")
            add_runtime_evidence(
                root,
                {"clients/rust/crates/rubin-node/src/sync_reorg.rs": ["runtime_reorg_coverage"]},
            )
    ...
    def test_runtime_evidence_rejects_rust_nested_external_module_stale_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            lib_rs = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "lib.rs"
            lib_rs.write_text("mod hidden {\n    pub mod sync_reorg;\n}\n", encoding="utf-8")
            add_runtime_evidence(
                root,
                {"clients/rust/crates/rubin-node/src/sync_reorg.rs": ["runtime_reorg_coverage"]},
            )
    ...
    def test_runtime_evidence_rejects_rust_cfg_disabled_nested_external_module_stale_file(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            lib_rs = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "lib.rs"
    ...
    def test_runtime_evidence_rejects_ignored_rust_test(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_rejects_rust_cfg_attr_ignore(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_allows_rust_cfg_test_function(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_allows_rust_cfg_attr_when_direct_test_remains(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
    ...
    def test_runtime_evidence_allows_rust_extended_test_function_forms(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text(
                '#[test]\nconst fn runtime_reorg_const() {}\n'
                '#[test]\nextern "C" fn runtime_reorg_extern() {}\n'
                "#[test]\nfn r#match() {}\n",
                encoding="utf-8",
            )
            captured = io.StringIO()
    ...
    def test_runtime_evidence_allows_rust_result_return_test(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text(
                "#[test]\nfn runtime_reorg_coverage() -> Result<(), String> { Ok(()) }\n",
                encoding="utf-8",
            )
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stdout(captured):
    ...
    def test_runtime_evidence_rejects_rust_test_with_parameter(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "sync_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text("#[test]\nfn runtime_reorg_coverage(arg: u8) {}\n", encoding="utf-8")
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
    ...
    def test_runtime_evidence_rejects_rust_test_with_unsupported_return_value(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "sync_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text("#[test]\nfn runtime_reorg_coverage() -> u8 { 1 }\n", encoding="utf-8")
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
    ...
    def test_runtime_evidence_rejects_rust_test_with_non_unit_result_return_value(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "sync_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text("#[test]\nfn runtime_reorg_coverage() -> Result<u8, String> { Ok(1) }\n", encoding="utf-8")
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
    ...
    def test_runtime_evidence_rejects_unsafe_rust_test_function(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
            ...
            rust_test = root / "clients" / "rust" / "crates" / "rubin-node" / "src" / "sync_reorg.rs"
            rust_test.parent.mkdir(parents=True, exist_ok=True)
            rust_test.write_text("#[test]\nunsafe fn runtime_reorg_coverage() {}\n", encoding="utf-8")
            captured = io.StringIO()
            with chdir(root), contextlib.redirect_stderr(captured):
                rc = m.main()
        self.assertEqual(rc, 1)
    ...
    def test_runtime_evidence_rejects_cfg_disabled_module_after_rust_char_brace(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            make_repo(root)
            add_runtime_evidence(
