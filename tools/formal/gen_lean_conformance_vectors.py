...
def _load_sighash_gate(path: Path, gate: str) -> list[SighashVector]:
    doc = json.loads(path.read_text(encoding="utf-8"))
...
def load_cv_sighash(path: Path) -> list[SighashVector]:
    return _load_sighash_gate(path, "CV-SIGHASH")


def render_sighash_gate_lean(vectors: list[SighashVector], gate: str, type_name: str, def_name: str) -> str:
    rows: list[str] = []
    for v in vectors:
        rows.append(
            "  { "
    ...
    module_body = (
        ...
        "\n"
        f"def {def_name} : List {type_name} := [\n"
        + body
        + "\n]\n"
        "\n"
    )
    return _render_lean_namespace(gate, module_body)


def render_cv_sighash_lean(vectors: list[SighashVector]) -> str:
    return render_sighash_gate_lean(vectors, "CV-SIGHASH", "CVSighashVector", "cvSighashVectors")
...
def _load_connect_block_gate(path: Path, gate: str, allowed_ops: tuple[str, ...]) -> list[SubsidyVector]:
    doc = json.loads(path.read_text(encoding="utf-8"))
    if doc.get("gate") != gate:
        raise ValueError(f"expected gate={gate}, got {doc.get('gate')!r}")
    vectors = doc.get("vectors")
    ...
    if not out:
        raise ValueError(f"no vectors found for {gate}")
    return out


def load_cv_subsidy(path: Path) -> list[SubsidyVector]:
    return _load_connect_block_gate(path, "CV-SUBSIDY", ("connect_block_basic", "block_basic_check_with_fees"))


def render_connect_block_gate_lean(
    vectors: list[SubsidyVector],
    gate: str,
    utxo_name: str,
    op_name: str,
    vector_name: str,
    ...
    return (
        ...
        "  createdByCoinbase : Bool\n"
        "\n"
        f"inductive {op_name} where\n"
        "  | connect_block_basic\n"
        "  | block_basic_check_with_fees\n"
        "deriving DecidableEq\n"
        "\n"
        f"structure {vector_name} where\n"
        "  id : String\n"
        f"  op : {op_name}\n"
        "  blockHex : String\n"
        "  expectedPrevHashHex : Option String\n"
        ...
        "  alreadyGenerated : Nat\n"
        "  sumFees : Option Nat\n"
        f"  utxos : List {utxo_name}\n"
        "  expectOk : Bool\n"
        "  expectErr : Option String\n"
        "\n"
        f"def {def_name} : List {vector_name} := [\n"
        + body
        + "\n]\n"
        "\n"
        "end RubinFormal.Conformance\n"
...
def render_cv_subsidy_lean(vectors: list[SubsidyVector]) -> str:
    return render_connect_block_gate_lean(
        vectors,
        "CV-SUBSIDY",
        "CVSubsidyUtxo",
        "CVSubsidyOp",
...
def render_cv_devnet_subsidy_lean(vectors: list[SubsidyVector]) -> str:
    rows: list[str] = []
    for v in vectors:
        sum_fees = f"some {v.sum_fees}" if v.sum_fees is not None else "none"
        rows.append(
    ...
    return (
        "-- AUTOGENERATED: do not edit by hand.\n"
        "-- Generated from conformance/fixtures/CV-DEVNET-SUBSIDY.json via tools/formal/gen_lean_conformance_vectors.py\n"
        "\n"
        "namespace RubinFormal.Conformance\n"
        "\n"
        "inductive CVDevnetSubsidyOp where\n"
        "  | connect_block_basic\n"
        "  | block_basic_check_with_fees\n"
        "deriving DecidableEq\n"
        "\n"
        "structure CVDevnetSubsidyVector where\n"
        "  id : String\n"
        "  op : CVDevnetSubsidyOp\n"
        "  blockHex : String\n"
        "  expectedPrevHashHex : Option String\n"
        "  expectedTargetHex : Option String\n"
        "  height : Nat\n"
        ...
        "  expectErr : Option String\n"
        "\n"
        "def cvDevnetSubsidyVectors : List CVDevnetSubsidyVector := [\n"
        + body
        + "\n]\n"
        "\n"
        "end RubinFormal.Conformance\n"
...
def main() -> int:
    ...
    dgen = _load_connect_block_gate(in_devnet_genesis, "CV-DEVNET-GENESIS", ("connect_block_basic",))
    out_devnet_genesis.write_text(
        _inject_perf_options(
            render_connect_block_gate_lean(
                dgen,
                "CV-DEVNET-GENESIS",
                "CVDevnetGenesisUtxo",
                "CVDevnetGenesisOp",
                "CVDevnetGenesisVector",
                "cvDevnetGenesisVectors",
            )
        ),
        encoding="utf-8",
    )
    print(f"WROTE: {out_devnet_genesis}")

    in_devnet_subsidy = repo_root / "conformance" / "fixtures" / "CV-DEVNET-SUBSIDY.json"
    out_devnet_subsidy = repo_root / "rubin-formal" / "RubinFormal" / "Conformance" / "CVDevnetSubsidyVectors.lean"
    dsub = _load_connect_block_gate(in_devnet_subsidy, "CV-DEVNET-SUBSIDY", ("connect_block_basic",))
    out_devnet_subsidy.write_text(
        _inject_perf_options(
            render_cv_devnet_subsidy_lean(dsub)
        ),
        encoding="utf-8",
    )
    print(f"WROTE: {out_devnet_subsidy}")

    in_devnet_chain = repo_root / "conformance" / "fixtures" / "CV-DEVNET-CHAIN.json"
    out_devnet_chain = repo_root / "rubin-formal" / "RubinFormal" / "Conformance" / "CVDevnetChainVectors.lean"
    dchain = _load_connect_block_gate(in_devnet_chain, "CV-DEVNET-CHAIN", ("connect_block_basic",))
    out_devnet_chain.write_text(
        _inject_perf_options(
            render_connect_block_gate_lean(
                dchain,
                "CV-DEVNET-CHAIN",
                ...
                "CVDevnetChainOp",
                "CVDevnetChainVector",
                "cvDevnetChainVectors",
            )
        ),
        encoding="utf-8",
    )
    print(f"WROTE: {out_devnet_chain}")

    in_devnet_maturity = repo_root / "conformance" / "fixtures" / "CV-DEVNET-MATURITY.json"
    out_devnet_maturity = repo_root / "rubin-formal" / "RubinFormal" / "Conformance" / "CVDevnetMaturityVectors.lean"
    dmat = _load_utxo_apply_vectors(json.loads(in_devnet_maturity.read_text(encoding="utf-8")), "CV-DEVNET-MATURITY")
    out_devnet_maturity.write_text(
        _inject_perf_options(render_utxo_apply_vectors_lean(dmat, "CV-DEVNET-MATURITY")),
        encoding="utf-8",
    )
    print(f"WROTE: {out_devnet_maturity}")

    in_devnet_sighash = repo_root / "conformance" / "fixtures" / "CV-DEVNET-SIGHASH-CHAINID.json"
    out_devnet_sighash = repo_root / "rubin-formal" / "RubinFormal" / "Conformance" / "CVDevnetSighashChainidVectors.lean"
    dsigh = _load_sighash_gate(in_devnet_sighash, "CV-DEVNET-SIGHASH-CHAINID")
    out_devnet_sighash.write_text(
        _inject_perf_options(
            render_sighash_gate_lean(
                dsigh,
                "CV-DEVNET-SIGHASH-CHAINID",
                "CVDevnetSighashChainidVector",
                "cvDevnetSighashChainidVectors",
            )
        ),
        encoding="utf-8",
    )
    print(f"WROTE: {out_devnet_sighash}")
    return 0
