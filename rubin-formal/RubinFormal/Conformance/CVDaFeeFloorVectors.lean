...
def cvDaFeeFloorVectors : List CVDaFeeFloorVector := [
  ...
  { id := "CV-DA-FEE-FLOOR-07-DA-FLOOR-OVERFLOW", fee := 18446744073709551615, weight := 1128, daBytes := 10, currentMempoolMinFeeRate := 1, minDaFeeRate := 18446744073709551615, daSurchargePerByte := 0, expectAdmit := false, expectAdmitClass := "rejected", expectDominantFloor := "da", expectRejectReason := some "DA_FEE_FLOOR_OVERFLOW", expectRelayFeeFloor := some 1128, expectDaFeeFloor := some 0, expectDaSurcharge := some 0, expectDaRequiredFee := some 0, expectRequiredFee := some 1128 },
