...
fn utxo_apply_missing_input_in_utxo_set() {
    ...
    if let Ok((tx, txid, _wtxid, _consumed)) = parse_tx(&tx_bytes) {
...
fn utxo_apply_with_matched_utxos_no_panic() {
    let tx_bytes = minimal_tx_bytes();
    if let Some((tx, txid, utxos)) = make_utxo_set_for_tx_inputs(&tx_bytes) {
        let chain_id = [0u8; 32];
...
fn minimal_tx_bytes() -> Vec<u8> {
    // Minimal standard tx: version=0, kind=0, 1 input, 1 output
    // This constructs bytes that parse_tx can accept.
    let mut buf = Vec::new();

    // version: u32 LE
    buf.extend_from_slice(&0u32.to_le_bytes());
    // tx_kind: 1 byte
    buf.push(0x00); // standard tx
                    // locktime: u64 LE
    ...
    buf.push(0x01);
    //   witness[0]: length (compact_size) + data
    buf.push(0x01); // 1 byte witness
    buf.push(0x00); // witness data
    ...
    //   covenant_data_len: compact_size
