...
#[test]
fn apply_non_coinbase_tx_basic_htlc_creation_first_error_order() {
    let prev_txid = [0x63u8; 32];
    let hash = [0x42u8; 32];
    ...
    let err =
        apply_non_coinbase_tx_basic(&tx, [0x64u8; 32], &utxos, 1, 0, ZERO_CHAIN_ID).unwrap_err();
    assert_eq!(err.code, ErrorCode::TxErrParse);
