...
fn check_refund_deferred(
    ...
    let (mut entry, mut path_item, sig_item, mut tx, input_index, input_value, chain_id, registry) =
        htlc_claim_fixture();
    ...
    let queue_err = validate_htlc_spend_q(
        &entry,
        &tx.witness[0],
        &tx.witness[1],
    ...
    .expect_err("refund selector must reject before queuing signature work");
    assert_eq!(queue_err.code, want);
    assert!(queue.is_empty(), "rejection retained queued signature work");
...
fn validate_htlc_refund_timelock_first_error_order_q() {
    ...
    check_refund_deferred(LOCK_MODE_HEIGHT, 10, 9, 0, false, &[], TxErrParse);
