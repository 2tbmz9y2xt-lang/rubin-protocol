...
pub fn verify_sig(
    ...
    match suite_id {
        SUITE_ID_ML_DSA_87 => openssl_verify_sig_message(alg, pubkey, signature, digest32),
        SUITE_ID_SLH_DSA_SHAKE_256F => {
            openssl_verify_sig_digest_oneshot(alg, pubkey, signature, digest32)
        }
        _ => unreachable!("suite_alg_name guards"),
    }
}

fn openssl_verify_sig_message(
    alg: &'static CStr,
    pubkey: &[u8],
    signature: &[u8],
    msg: &[u8],
    ...
    unsafe {
        ...
        if ctx.is_null() {
            ...
            return Err(TxError::new(
                ErrorCode::TxErrParse,
                "openssl: EVP_PKEY_CTX_new failed",
            ));
        }
