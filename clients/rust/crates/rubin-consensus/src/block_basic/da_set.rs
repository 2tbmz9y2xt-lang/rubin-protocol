...
fn validate_da_set_structure(sets: &DaSetStructure) -> Result<(), TxError> {
    ...
    validate_da_duplicate_commits(&sets.duplicate_commit_ids)?;
    validate_da_set_completeness(sets)?;
    validate_da_batch_count(&sets.commits)?;
    validate_da_chunk_count_bounds(&sets.commits)
}

fn validate_da_duplicate_commits(duplicate_commit_ids: &BTreeSet<[u8; 32]>) -> Result<(), TxError> {
    if !duplicate_commit_ids.is_empty() {
        return Err(TxError::new(
...
fn validate_da_set_completeness(sets: &DaSetStructure) -> Result<(), TxError> {
    for (da_id, commit) in &sets.commits {
        validate_da_duplicate_chunk_indexes(&sets.duplicate_chunk_indexes, da_id)?;
        validate_da_chunk_set_completeness(&sets.chunks, da_id, commit.chunk_count)?;
...
fn validate_da_duplicate_chunk_indexes(
    duplicate_chunk_indexes: &BTreeMap<[u8; 32], BTreeSet<u16>>,
    da_id: &[u8; 32],
) -> Result<(), TxError> {
    if duplicate_chunk_indexes
        .get(da_id)
...
fn validate_da_chunk_set_completeness(
    chunks: &DaChunkMap,
    da_id: &[u8; 32],
    chunk_count: u16,
) -> Result<(), TxError> {
...
fn validate_da_chunk_count_bounds(commits: &DaCommitMap) -> Result<(), TxError> {
...
fn validate_da_chunk_indexes(set: &BTreeMap<u16, Tx>, chunk_count: u16) -> Result<(), TxError> {
...
fn da_chunk_set_for_id<'a>(
    chunks: &'a DaChunkMap,
    da_id: &[u8; 32],
) -> Result<&'a BTreeMap<u16, Tx>, TxError> {
    chunks
        .get(da_id)
        .ok_or_else(|| TxError::new(ErrorCode::BlockErrDaIncomplete, "DA commit without chunks"))
}

fn da_chunk_tx_for_index(set: &BTreeMap<u16, Tx>, index: u16) -> Result<&Tx, TxError> {
    set.get(&index)
...
mod tests {
    ...
    fn da_commit_for_id_missing_returns_block_err_da_set_invalid() {
        let commits: BTreeMap<[u8; 32], DaCommitSet> = BTreeMap::new();
    ...
    fn da_chunk_set_for_id_missing_returns_block_err_da_incomplete() {
        let chunks: BTreeMap<[u8; 32], BTreeMap<u16, Tx>> = BTreeMap::new();
    ...
    fn da_chunk_tx_for_index_missing_returns_block_err_da_incomplete() {
        let set: BTreeMap<u16, Tx> = BTreeMap::new();
    ...
    fn validate_da_commit_completeness_batch_exceeded_fires_before_set_invalid() {
        ...
        let mut orphan_set = BTreeMap::new();
