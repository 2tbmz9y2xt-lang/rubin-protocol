...
fn run(args: &[String], stdout: &mut dyn Write, stderr: &mut dyn Write) -> i32 {
    ...
    // Fail early if non-devnet network uses default (zeroed) genesis — handshakes
    // against real peers will always fail with mismatched chain_id/genesis_hash.
    if cfg.network != "devnet" && chain_id == [0u8; 32] && cfg.genesis_file.is_none() {
        let _ = writeln!(
            stderr,
