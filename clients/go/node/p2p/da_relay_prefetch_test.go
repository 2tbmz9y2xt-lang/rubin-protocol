...
func TestDAPrefetchPlansFromCurrentRecord(t *testing.T) {
	state := newDARelayStateForTest(t, defaultDARelayCaps())
	daID := daRelayTestID(136)
	stale := mustAddDACommit(t, state, "", daRelayTestCommit(daID, 4, 1))
	mustAddDAChunk(t, state, "peer-a", daRelayTestChunk(daID, 0, 1))

	plans, diagnostic := state.planDAPrefetch(stale, []string{"peer-a"}, time.Unix(1000, 0))
	total, _, _ := summarizeDAPrefetchPlans(plans)
	if total != 3 || diagnostic != "" {
		t.Fatalf("total=%d diagnostic=%q, want only currently missing chunks", total, diagnostic)
	}
	if _, ok := state.prefetch.indexes[daID][0]; ok {
		t.Fatalf("reservations=%v, want no stale reservation for fulfilled chunk", state.prefetch.indexes[daID])
	}
}
...
func requireDAPrefetchCompleteCleanup(t *testing.T, state *daRelayState, record daRelaySetRecord, keys []string, now time.Time) {
	...
	current.state = daRelayStateCompleteSet
	state.mu.Lock()
	state.sets[record.daID] = current
	state.mu.Unlock()
	plans, diagnostic := state.planDAPrefetch(record, keys, now)
