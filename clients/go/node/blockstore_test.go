...
// (writeFileAtomicFn, atomicWriteIO): none may call t.Parallel.
// ---------------------------------------------------------------------------
...
func TestPreparedCanonicalIndexAcceptedTransitions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			reopened := mustOpenBlockStore(t, root)
			assertPublishedCanonicalImage(t, reopened, tc.next, prepared.newRaw)
...
// test's own oracle for "same sequence, different bytes", never produced by the
// code under test, and every user proves it differs from the canonical encoding so
// no row can pass because the two spellings collapsed.
func respellCanonicalIndex(t *testing.T, rows ...string) []byte {
	...
	return mustMarshalStoreEnvelope(t, `{"version":1,"canonical":`+inner+`}`)
}
...
// withLoadBlockStoreIndexFn replaces the strict readback seam so a test can count
// the reads one commit performs. It mirrors withWriteFileAtomicFn.
...
// countCanonicalWrites counts write attempts and reports success without touching
// the disk, for lanes that only care how many times the write lane ran.
...
// TestPreparedCanonicalIndexSequenceIdentityPreparationAndFreshness: preparation
// and commit freshness compare the complete ordered ROW SEQUENCE of spec §6.4.1 —
// length and every row, position = height — never the bytes that spell it. So a
// re-spelled visible identity is the SAME identity, a truly moved or undecodable
// one is refused before the single-use claim and before any write, and an image
// with NO comparison identity still saves through Restore yet may never commit.
func TestPreparedCanonicalIndexSequenceIdentityPreparationAndFreshness(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "malformed_envelope_old_raw", oldRaw: []byte("bytes this store never had"), next: nextRows},
	...
	for _, tc := range []struct {
		...
		// A legacy writer that re-encoded the same rows leaves exactly this cache.
		{name: "respelled_cached_identity_is_fresh", cache: setCache(respelledSeed), wantClass: canonicalCommitted, wantWrites: 1},
		// The EMPTY visible sequence is the configuration that needs the explicit
		// no-identity guard: a nil identity compares equal to an empty one, so a
		// freshness check that only compared sequences would let a Restore-shaped
		// image commit against a fresh store.
		...
		// Empty again for the fail-closed arm: an undecodable cache decodes to NO
		// sequence, which compares equal to the empty one, so only an explicit
		// decode-failure refusal keeps a corrupt cache from reading as fresh.
	...
	// Restore has NO comparison identity, so an identical list is not a no-op and
	// must still reach the save. The empty row is the one that matters: treating a
	// nil identity as a comparison identity makes it equal to the empty sequence.
	...
	t.Run("concurrent_double_commit_claims_once", func(t *testing.T) {
		...
		// Both goroutines park on the barrier before either may claim, so the CAS
		// is exercised under real contention (the gate runs this under -race).
...
// TestPreparedCanonicalIndexPostCommitVisibleSequenceClassify: whenever the write
// may have crossed the namespace commit — the lane said so, or it returned an error
// it never classified at all — exactly ONE strict readback classifies by the
// complete ordered ROW SEQUENCE. Two spellings of one sequence are ONE identity, so
// a re-spelled old or new image classifies as that identity and terminal NEW caches
// the EXACT bytes the disk holds; a shared tip, a reordering, a duplicate, a prefix,
// an extra row, a matching length, a missing file and an unreadable one are each
// NEITHER. The error text never classifies, a stale RAM identity never classifies,
// and a proven pre-commit failure never reads at all. Every row also pins single
...
func TestPreparedCanonicalIndexPostCommitVisibleSequenceClassify(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		wantReads   int
		wantReadErr bool
		// wantRaw non-nil means publication is expected, and it is the EXACT
		// visible identity the store must cache — a literal fixture, never a
		// re-encoding derived from the image under test.
		wantRaw []byte
		alsoIs  error
	}{
		...
		// Decodes fine — validateBlockStoreIndex has no duplicate-row check — so
		// only sequence inequality refuses it.
		{name: "duplicated_new_row", visible: plantRows(row0, row1, row1), wantClass: canonicalCommitTerminalUnknown, wantReads: 1},
