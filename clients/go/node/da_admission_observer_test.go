...
func observeDANodePlanner(id string, raw json.RawMessage) (map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("observer input %s: planner: %w", id, err)
	}
...
func projectDANodePlanner(plan daCompleteCapacityPlan, inputUnchanged bool) map[string]any {
	...
	for i, victim := range plan.victims {
		victims[i] = daNodeObserverHexID(victim)
	}
...
func requireDANodeObserverCaseOrder(cases []daNodeObserverCase) error {
	if len(cases) != len(daNodeObserverIDs) {
		return fmt.Errorf("observer census: found %d assigned cases, want %d", len(cases), len(daNodeObserverIDs))
	}
	for i, want := range daNodeObserverIDs {
		if cases[i].ID != want {
			return fmt.Errorf("observer census: case %d is %q, want %q", i, cases[i].ID, want)
		}
		if len(cases[i].Input) == 0 || bytes.Equal(cases[i].Input, []byte("null")) {
			return fmt.Errorf("observer input %s: missing input", want)
		}
	}
	return nil
}
...
func TestDAAdmissionObserverNodeProjection(t *testing.T) {
	...
	if !reflect.DeepEqual(accepted, wantAccepted) {
		t.Fatalf("observer projection accepted=%v", accepted)
	}
	...
	if !reflect.DeepEqual(refused, wantRefused) {
		t.Fatalf("observer projection refusal=%v", refused)
	}
	plan.victims[0][31] = 9
...
func TestDAAdmissionObserverNodeAlias(t *testing.T) {
	...
	if saved.residents[0].id[0] != 1 {
		t.Fatal("observer alias: source mutation changed saved resident copy")
	}
	saved.residents[0].fee.Lo = 8
	if input.residents[0].fee.Lo != 3 {
		t.Fatal("observer alias: copy mutation changed source resident")
	}
}
...
func poisonDANodeObserverExpectations(raw []byte) ([]byte, error) {
	...
	if expectCount == 0 {
		return nil, fmt.Errorf("observer expected isolation: corpus has no expect fields")
	}
	for field, count := range annotationCounts {
		if count == 0 {
			return nil, fmt.Errorf("observer expected isolation: annotation %s absent", field)
		}
	}
	...
	for _, receipt := range []string{"forward_receipt", "reverse_receipt"} {
		if len(root[receipt]) == 0 {
			return nil, fmt.Errorf("observer expected isolation: missing %s", receipt)
		}
...
func TestDAAdmissionObserverNodeExpectedIsolation(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("observer expected isolation input: %v", err)
	}
	before, err := collectDANodeObserver(raw)
	if err != nil {
		t.Fatalf("observer expected isolation baseline: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("observer expected isolation poisoned collection: %v", err)
	}
	if !bytes.Equal(before, after) {
		t.Fatal("observer expected isolation: expected/receipt/annotation mutation changed actual bytes")
	}
}
...
func mutateDANodeObserverInput(t *testing.T, raw []byte, caseID string, mutate func(map[string]json.RawMessage) error) []byte {
	...
	if found != 1 {
		t.Fatalf("observer census: mutation target %s count=%d", caseID, found)
	}
	encodedCases, err := json.Marshal(cases)
	if err != nil {
		t.Fatal(err)
...
func TestDAAdmissionObserverNodeInputReachability(t *testing.T) {
	_, raw, err := loadDANodeObserverCorpus(daNodeObserverCorpusPath())
	if err != nil {
		t.Fatalf("observer input reachability corpus: %v", err)
	}
	baseBytes, err := collectDANodeObserver(raw)
	if err != nil {
		t.Fatalf("observer input reachability baseline: %v", err)
	}
	...
	for _, id := range daNodeObserverIDs {
		...
		if id != "CAP_CANDIDATE_WINS_EQUALITY_REJECT" && plan["accepted"] != true {
			t.Fatalf("observer input reachability %s: frozen plan was not accepted", id)
		}
		mutated := mutateDANodeObserverInput(t, raw, id, func(input map[string]json.RawMessage) error {
			...
			field := "total_bytes"
			if id == "CAP_CANDIDATE_WINS_EQUALITY_REJECT" {
				field = "total_fee"
			}
			var decimal string
			if err := json.Unmarshal(candidate[field], &decimal); err != nil {
				return fmt.Errorf("observer input reachability %s: %w", id, err)
			}
			value, err := strconv.ParseUint(decimal, 10, 64)
			if err != nil || value == ^uint64(0) {
				return fmt.Errorf("observer input reachability %s: candidate increment overflows", id)
			}
		...
		if err != nil {
			t.Fatalf("observer input reachability %s: %v", id, err)
		}
		var changed daNodeObserverOutput
		if err := json.Unmarshal(changedBytes, &changed); err != nil {
			t.Fatal(err)
		}
		if bytes.Equal(observerActualBytes(t, base, id), observerActualBytes(t, changed, id)) {
			t.Fatalf("observer input reachability %s: actual row unchanged", id)
		}
	}
}
...
func TestDAAdmissionObserverNodeIntegrity(t *testing.T) {
	_, raw, err := loadDANodeObserverCorpus(daNodeObserverCorpusPath())
	if err != nil {
		t.Fatalf("observer integrity corpus: %v", err)
	}
	unchanged := bytes.Clone(raw)
	if _, err := collectDANodeObserver(raw); err != nil {
		t.Fatalf("observer integrity baseline: %v", err)
	}
	if !bytes.Equal(raw, unchanged) {
		t.Fatal("observer alias: collection changed source corpus bytes")
	}
	for label, changed := range map[string][]byte{
		...
		if _, err := collectDANodeObserver(changed); err == nil || !strings.Contains(err.Error(), "observer census") {
			t.Fatalf("observer census %s error=%v", label, err)
		}
	}
	...
	mutations := []struct {
		...
		{"missing candidate class ordinal", "CAP_RETAINED_BYTES_BELOW", "missing required identity ordinal", func(in map[string]json.RawMessage) error {
			return editDANodeObserverNested(in, "candidate", func(v map[string]json.RawMessage) error { delete(v, "class_ordinal"); return nil })
		}},
		{"missing resident ordinal", "CAP_RETAINED_BYTES_BELOW", "missing required identity ordinal", func(in map[string]json.RawMessage) error {
			return editDANodeObserverNested(in, "candidate", func(v map[string]json.RawMessage) error { delete(v, "resident_ordinal"); return nil })
		}},
		{"missing resident class ordinal", "CAP_RETAINED_BYTES_BELOW", "missing required resident ordinal", func(in map[string]json.RawMessage) error {
			return editDANodeObserverResident(in, 0, func(v map[string]json.RawMessage) error { delete(v, "class_ordinal"); return nil })
		}},
		{"missing range first ordinal", "CAP_RETAINED_BYTES_BELOW", "missing required resident ordinal", func(in map[string]json.RawMessage) error {
			return editDANodeObserverResident(in, 0, func(v map[string]json.RawMessage) error {
				return editDANodeObserverNested(v, "resident_ordinal_range", func(r map[string]json.RawMessage) error { delete(r, "first"); return nil })
			})
		}},
		...
		{"capacity width", "CAP_RETAINED_BYTES_BELOW", "effective_da_mempool_size", func(in map[string]json.RawMessage) error {
			in["effective_da_mempool_size"] = json.RawMessage(`"18446744073709551616"`)
			return nil
		}},
		{"complete set bound", "CAP_RETAINED_BYTES_BELOW", "unsupported planner bounds", func(in map[string]json.RawMessage) error {
			in["complete_set_max_count"] = json.RawMessage(`65535`)
			return nil
		}},
		{"pinned payload bound", "CAP_RETAINED_BYTES_BELOW", "unsupported planner bounds", func(in map[string]json.RawMessage) error {
			in["pinned_payload_max"] = json.RawMessage(`"95999999"`)
			return nil
		}},
		...
		{"sequence progression overflow", "CAP_SET_COUNT_BELOW", "resident sequence overflow", func(in map[string]json.RawMessage) error {
			return editDANodeObserverResident(in, 0, func(r map[string]json.RawMessage) error {
				r["received_sequence_first"] = json.RawMessage(`"18446744073709551615"`)
				r["received_sequence_step"] = json.RawMessage(`"1"`)
				return nil
			})
		}},
		{"fee progression overflow", "CAP_SET_COUNT_BELOW", "resident fee overflow", func(in map[string]json.RawMessage) error {
			return editDANodeObserverResident(in, 0, func(r map[string]json.RawMessage) error {
				r["total_fee"] = json.RawMessage(`"340282366920938463463374607431768211455"`)
				r["total_fee_step"] = json.RawMessage(`"1"`)
				return nil
			})
		}},
...
func editDANodeObserverNested(input map[string]json.RawMessage, field string, edit func(map[string]json.RawMessage) error) error {
	...
	if err := json.Unmarshal(input[field], &nested); err != nil {
		return err
	}
	if err := edit(nested); err != nil {
		return err
	}
...
func editDANodeObserverResident(input map[string]json.RawMessage, index int, edit func(map[string]json.RawMessage) error) error {
	return editDANodeObserverNested(input, "logical_prestate", func(logical map[string]json.RawMessage) error {
		...
		if index < 0 || index >= len(residents) {
			return fmt.Errorf("resident index %d out of range", index)
		}
		var resident map[string]json.RawMessage
		if err := json.Unmarshal(residents[index], &resident); err != nil {
			return err
		}
		if err := edit(resident); err != nil {
			return err
		}
...
func rangeEndpointDANodeObserverMutation(input map[string]json.RawMessage) error {
	...
	if planner.CaseOrdinal == nil || len(planner.LogicalPrestate.ResidentClasses) == 0 || planner.LogicalPrestate.ResidentClasses[0].ClassOrdinal == nil || planner.LogicalPrestate.ResidentClasses[0].ResidentOrdinalRange.First == nil {
		return fmt.Errorf("observer input range mutation: missing ordinals")
	}
...
func residentBoundDANodeObserverMutation(input map[string]json.RawMessage) error {
	...
	if planner.CaseOrdinal == nil || len(planner.LogicalPrestate.ResidentClasses) == 0 || planner.LogicalPrestate.ResidentClasses[0].ClassOrdinal == nil || planner.LogicalPrestate.ResidentClasses[0].ResidentOrdinalRange.First == nil {
		return fmt.Errorf("observer input bound mutation: missing ordinals")
	}
	first := *planner.LogicalPrestate.ResidentClasses[0].ResidentOrdinalRange.First
	if first > ^uint64(0)-65536 {
		return fmt.Errorf("observer input bound mutation: ordinal overflow")
	}
	last := first + 65536
...
func duplicateDANodeObserverResident(input map[string]json.RawMessage) error {
	return editDANodeObserverNested(input, "logical_prestate", func(logical map[string]json.RawMessage) error {
		...
		if len(residents) == 0 {
			return fmt.Errorf("no resident class to duplicate")
		}
...
func observerCaseID(t *testing.T, raw json.RawMessage) string {
	...
	if err := json.Unmarshal(raw, &row); err != nil {
		t.Fatalf("observer census case: %v", err)
	}
	return row.ID
}
...
func findDANodeObserverCase(t *testing.T, cases []json.RawMessage, id string) json.RawMessage {
	...
	for _, row := range cases {
		if observerCaseID(t, row) == id {
			return bytes.Clone(row)
		}
	}
...
func swapDANodeObserverCases(t *testing.T, cases []json.RawMessage, first, second string) []json.RawMessage {
	...
	if firstIndex < 0 || secondIndex < 0 {
		t.Fatalf("observer census: missing cases %s/%s", first, second)
	}
...
func selectDANodeObserverCases(corpus daNodeObserverCorpus) ([]daNodeObserverCase, error) {
	...
	if err := requireDANodeObserverCaseOrder(selected); err != nil {
		return nil, err
	}
	return selected, nil
}
...
func collectDANodeObserver(raw []byte) ([]byte, error) {
	...
	for _, c := range cases {
		...
		if observed.ID != c.ID || observed.Actual == nil {
			return nil, fmt.Errorf("observer input %s: incomplete actual record", c.ID)
		}
...
func TestDAAdmissionObserverNode(t *testing.T) {
	...
	if output.FormatVersion != 1 || len(output.Cases) != len(daNodeObserverIDs) {
