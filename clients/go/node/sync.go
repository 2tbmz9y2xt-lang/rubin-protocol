...
func (s *SyncEngine) ClaimDARelayState(expectedRelay *DARelayState, admitDetachedReorg func([]byte) error) (*DARelayState, error) {
	...
	if expectedRelay == nil || expectedRelay != s.daRelay {
		return nil, errors.New("sync engine DA relay state binding mismatch")
	}
	if admitDetachedReorg == nil {
		return nil, errors.New("sync engine detached DA admission is not initialized")
	}
	s.reorgDAAdmission = admitDetachedReorg
...
// allocating step. FIXED_PUBLICATION forbids formatting inside the corridor, so
// the corridor's caller builds the string through this first, then only stores.
