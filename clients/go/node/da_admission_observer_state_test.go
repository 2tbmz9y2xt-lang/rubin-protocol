...
type daNodeObserverStateMember struct {
	...
	ChunkCount  uint16
	ChunkHash   [32]byte
	// Token is the owner TokenHighWater read right after this member's setup admission.
	Token uint64
}
...
func validateDANodeObserverStateMixedProfile(p daNodeObserverStateMixedProfile) error {
	if p.MemberTemplateRef != "CANONICAL_MEMBER" || p.ClassOrdinal != 0 || p.ResidentOrdinal != 0 ||
		...
		p.ChunkCount != 3 || !slices.Equal(p.ChunkLengths, []uint64{
		10,
		20,
		1,
	}) ||
		...
		!slices.Equal(p.AdmissionOrder, []uint64{
			...
			1,
			2,
		}) || !slices.Equal(p.RetainedMemberOrdinals, []uint64{
		0,
		1,
		2,
	}) ||
		!slices.Equal(p.AbsentMemberOrdinals, []uint64{3}) || !slices.Equal(p.AdmissionOrdinals, []uint64{
		1,
		2,
		3,
	}) {
...
func newDANodeObserverStateFixture(row daNodeObserverStateCase, profile daNodeObserverStateProfile) (fixture *daNodeObserverStateFixture, err error) {
	...
	cfg.MinDaFeeRate = minFee
	cfg.PolicyDaSurchargePerByte = surcharge
	cfg.PolicyMaxDaBytesPerBlock = maxDABytes
	...
	fixture = &daNodeObserverStateFixture{
		...
		Mempool: mp,
		Signer:  signer,
		Members: make(map[daNodeObserverStateMemberKey]daNodeObserverStateMember, 320),
	}
	if row.Input.ExecutionBoundary == "COMPLETE_COMMIT_PRECONDITION" {
		...
		class := daNodeObserverStateResidentClass{
			ClassOrdinal: mixed.ClassOrdinal,
			ChunkCount:   mixed.ChunkCount,
			ChunkLengths: mixed.ChunkLengths,
			PayloadByte:  mixed.PayloadByte,
			CommitFee:    mixed.CommitFee,
			ChunkFee:     mixed.ChunkFee,
		}
...
func buildDANodeObserverStateMemberOnce(f *daNodeObserverStateFixture, row daNodeObserverStateCase, p daNodeObserverStateProfile, address []byte, key daNodeObserverStateMemberKey, admissionOrdinal, fee uint64, kind uint8, chunkCount, chunkIndex uint64, payload []byte, commitment [32]byte) (daNodeObserverStateMember, error) {
	...
	if kind == 0x01 {
		...
		out.ChunkCount = uint16(chunkCount)
...
func setupDANodeObserverStateSet(f *daNodeObserverStateFixture, row daNodeObserverStateCase, p daNodeObserverStateProfile, address []byte, class daNodeObserverStateResidentClass, resident, ordinal, count, retained uint64, provenance []DAProvenance, equality bool) ([]daNodeObserverStateMember, error) {
	...
	for i := uint64(0); i < count; i++ {
		...
		if i < retained {
			...
			owner.mu.Lock()
			member.Token = owner.tokenHighWater
			owner.mu.Unlock()
...
func parseDANodeObserverStateProvenance(raw json.RawMessage) ([]DAProvenance, error) {
	...
	for i, member := range members {
		...
		case "LOCAL", "DETACHED_REORG":
			...
			out[i] = map[string]DAProvenance{
				"LOCAL":          LocalDAProvenance(),
				"DETACHED_REORG": DetachedReorgDAProvenance(),
			}[source]
		case "PEER":
...
func stateSetupBaseline(f *daNodeObserverStateFixture) error {
	...
	records := make(map[[32]byte]bool)
	members := 0
	for key, member := range f.Members {
		records[member.DAID] = true
		if !daNodeObserverStateCandidateMatches(image, member, observed.Owner.Generation, member.Token) {
...
func applyDANodeObserverStatePlannedControl(f *daNodeObserverStateFixture, control daNodeObserverStateControl) (func() error, error) {
	...
	case "CORRUPT_RESIDENT_LOCATOR_DA_ID":
		...
		corrupt := daRelayLocator{
			daID:       control.locator,
			kind:       locator.kind,
...
func admitDANodeObserverStateCandidate(f *daNodeObserverStateFixture, control daNodeObserverStateControl) (daNodeObserverStateAdmission, error) {
	...
	f.Relay.completeHook = func(stage daCompleteStage, plan *daCompleteCommitPlan) {
		...
		phase := map[daCompleteStage]string{
			daCompletePlanned: "PLANNED",
			daCompleteEffects: "EFFECTS",
		}[stage]
...
func daNodeObserverStateCandidateMatches(image DAObserverStateImage, candidate daNodeObserverStateMember, generation, token uint64) bool {
	...
	slotMatches := record.Commit.DAID == candidate.DAID &&
		record.Commit.PayloadCommitment == candidate.Commitment &&
		record.Commit.ChunkCount == candidate.ChunkCount
	if candidate.Key.MemberOrdinal != 0 {
		chunk := record.Chunks[slices.IndexFunc(record.Chunks, func(chunk DAObserverChunk) bool {
			return chunk.Member == member
		})]
		slotMatches = chunk.DAID == candidate.DAID &&
			chunk.ChunkIndex == candidate.ChunkIndex &&
			chunk.ChunkHash == candidate.ChunkHash
	}
	if !slotMatches || member.TokenSeq != token {
		return false
	}
...
func daNodeObserverStateOwnerOutput(invocations uint64, counts DAObserverOwnerCounts, highWater uint64, releaseScope string) map[string]any {
	return map[string]any{
		"admission_entrypoint": map[string]any{
			"domain":      "DA",
...
func validateDANodeObserverStateInput(id string, raw json.RawMessage) (daNodeObserverStateInput, error) {
	...
	ref, known := map[string]string{
		"COMPLETE_COMMIT_PRECONDITION":   "COMPLETE_COMMIT_7_SETS",
		"PEER_CLEANUP_STEP_PRECONDITION": "STATE_B_MIXED",
	...
	fields := []stateRawField{
		...
		{
			"member_provenance",
			input.MemberProvenance,
			!complete,
		},
		{
			"cleanup_selector",
			input.CleanupSelector,
			!complete,
		},
	}
	...
	if complete {
		...
		labeled, ok := map[string]uint64{
			"ACCEPTED_SEQUENCE_MAX":           ^uint64(0),
			"ACCEPTED_SEQUENCE_MAX_MINUS_ONE": ^uint64(0) - 1,
		}[control.action]
...
// In each after observation (odd index) the candidate TokenSeq and the minted owner TokenHighWater are one source.
func stateDistinctSources(invocations []uint64, candidate daNodeObserverStateMember, observations []daNodeObserverStateObservation, coupled ...string) error {
	seen := make(map[any]string)
	errs := make([]error, 0)
	put := func(prefix, names string, values ...any) {
		for i, name := range strings.Fields(names) {
			name := prefix + name
			value := values[i]
			other, dup := seen[value]
			isCoupled := slices.ContainsFunc(coupled, func(field string) bool {
				return strings.HasSuffix(name, " "+field) && strings.HasSuffix(other, " "+field)
			})
			if dup && other != name && !isCoupled && !reflect.ValueOf(value).IsZero() {
				errs = append(errs, fmt.Errorf("%s and %s share %v", other, name, value))
			}
			seen[value] = name
		}
	}
	...
	images := make([]DAObserverStateImage, 0)
	owners := make([]daNodeObserverStateOwnerImage, 0)
	for i, observation := range observations {
		...
		if !slices.Contains(owners, owner) {
			...
			put(
				...
				"ReserveCalls ReservationsAcquired Finalizations CandidateReleases",
				owner.Counts.ReserveCalls,
				owner.Counts.ReservationsAcquired,
				owner.Counts.Finalizations,
				owner.Counts.CandidateReleases,
			)
			if i%2 == 0 || owner.TokenHighWater != daNodeObserverStateCandidateToken(observation.Image, candidate) {
				put(fmt.Sprintf("owner %d ", i), "TokenHighWater", owner.TokenHighWater)
			}
...
func TestDAAdmissionObserverNodeStateIntegrity(t *testing.T) {
	...
	unknownCommit := cases[0].Case
	unknownCleanup := cases[12].Case
	unknownCommit.ID = "UNKNOWN_STATE_CASE"
	unknownCleanup.ID = "UNKNOWN_STATE_CASE"
	cleanupAsCommit := cases[0].Case
	...
	for mode, probe := range []struct {
		...
		{
			func(c []json.RawMessage) []json.RawMessage { return c[:len(c)-1] },
			"observer census: got 12 owned cases",
		},
		{
			func(c []json.RawMessage) []json.RawMessage { return append(c, owned[0]) },
			"observer census: duplicate or out-of-order",
		},
		{
		...
		{
			func(c []json.RawMessage) []json.RawMessage {
				c[0], _ = json.Marshal(unknownCommit)
				return c
			},
			"observer census: unknown owned case",
		},
		{
			func(c []json.RawMessage) []json.RawMessage {
				c[12], _ = json.Marshal(unknownCleanup)
				return c
			},
			"observer census: unknown owned case",
		},
		{
			func(c []json.RawMessage) []json.RawMessage {
				c[0], _ = json.Marshal(cleanupAsCommit)
				return c
			},
			`case "CAP_ALL_MEMBERS_REMOVED" in the wrong execution_boundary`,
		},
		{
			func(c []json.RawMessage) []json.RawMessage {
				c[12], _ = json.Marshal(commitAsCleanup)
				return c
			},
	...
	for _, probe := range [][2]string{
		{
			`{"phase":"UNKNOWN","action":"NONE"}`,
			"control: unknown phase/action",
		},
		{
			`{"phase":"NONE","action":"UNKNOWN"}`,
			"control: unknown phase/action",
		},
		{
			...
			"control.preserve_other_fields: must be true",
		},
		{
			`{"phase":"NONE","action":"NONE","member":null}`,
			"control.member: missing or forbidden",
		},
		{
			`{"phase":"NONE","action":"NONE","extra":1}`,
			`unknown field "extra"`,
		},
		{
		...
		},
		{
			...
			"locator_da_id: expected 32-byte hex",
		},
		{
			...
			"intrinsic_total_bytes_delta: invalid decimal",
		},
		{
			`{"phase":"PLANNED","action":"INCREMENT_RESIDENT_INTRINSIC_TOTAL_BYTES","member":{"class_ordinal":0,"resident_ordinal":0,"member_ordinal":0},"preserve_other_fields":true,"intrinsic_total_bytes_delta":"0"}`,
			"control.intrinsic_total_bytes_delta: must be positive",
		},
		{
			`{"phase":"PLANNED","action":"ADVANCE_TARGET_AND_GLOBAL_REVISION","preserve_other_fields":true,"target_revision":"global_record_revision","global_record_revision":"global_record_revision + 1"}`,
			"control.target_revision: unsupported expression",
		},
		{
			`{"phase":"PLANNED","action":"ADVANCE_TARGET_AND_GLOBAL_REVISION","preserve_other_fields":true,"target_revision":"global_record_revision + 1","global_record_revision":"global_record_revision + 2"}`,
			"control.global_record_revision: unsupported expression",
		},
	} {
	...
	for _, probe := range [][2]string{
		{
			`[{"source":"UNKNOWN"},{"source":"LOCAL"},{"source":"DETACHED_REORG"}]`,
			"member_provenance: unknown source",
		},
		{
			`[{"source":"PEER"},{"source":"LOCAL"},{"source":"DETACHED_REORG"}]`,
			"member_provenance.peer_identity: missing string",
		},
		{
			`[{"source":"LOCAL","peer_identity":"x"},{"source":"LOCAL"},{"source":"DETACHED_REORG"}]`,
			"member_provenance[0]: forbidden peer identities",
		},
	} {
	...
	for _, probe := range []struct {
	...
	}{
		{
			...
			"STATE_B_MIXED: unsupported cleanup profile",
		},
		{
		...
		},
		{
			...
			"COMPLETE_COMMIT_7_SETS: invalid target",
		},
		{
			func(p *daNodeObserverStateProfile) { p.Canonical.Input.Value = "18446744073709551616" },
			"CANONICAL_MEMBER: unsupported signed-member profile",
		},
		{
	...
	overflow := daNodeObserverStateControl{
		action:   "INCREMENT_RESIDENT_INTRINSIC_TOTAL_BYTES",
		selected: &daNodeObserverStateMemberKey{},
		delta:    ^uint64(0),
	}
	...
	seq := profile
	seq.Mixed.FirstReceivedSequence = "2"
	seq.Complete.AcceptedSequence = "317"
	for _, row := range []daNodeObserverStateCase{
		cases[11],
		cases[0],
	} {
...
func TestDAAdmissionObserverNodeStateProjection(t *testing.T) {
	...
	small := &daNodeObserverStateFixture{
		TargetCommit: commit,
		Candidate:    chunk,
	}
	cm := &DAObserverMember{
	...
	build := func(received uint64, candidate []byte, victims ...byte) DAObserverStateImage {
		...
		for _, id := range victims {
			...
			image.Locators = append(image.Locators, DAObserverLocator{
				TxID: member,
				DAID: [32]byte{id},
			})
		...
		if candidate != nil {
			...
			image.OutpointRows = []DAObserverOutpointRow{{
				Outpoint: chunk.Inputs[0],
				TokenSeq: 3000,
				TxID:     chunk.TxID,
			}}
		}
	...
	// minted sets the candidate token that the run's reservation minted.
	minted := func(image DAObserverStateImage, token uint64) DAObserverStateImage {
		target := &image.Records[len(image.Records)-1]
		member := *target.Chunks[0].Member
		member.TokenSeq = token
		target.Chunks[0].Member = &member
		image.Claims[len(image.Claims)-1].TokenSeq = token
		image.OutpointRows[0].TokenSeq = token
		return image
	}
	...
	admission := func(invocations uint64, err error, before, after DAObserverStateImage, from, to daNodeObserverStateOwnerImage, plan ...byte) daNodeObserverStateAdmission {
	...
	published := ownerAt(205, 306, 407, 500, 1016)
	refused := ownerAt(205, 306, 407, 510, 1016)
	fresh := admission(15, nil, build(61, nil, 0x41, 0x42), minted(build(26, []byte{
		2,
		3,
	}), 1027), published, ownerAt(215, 317, 419, 500, 1027), 0x42, 0x41)
	repeatBefore := ownerAt(600, 700, 800, 900, 2000)
	repeated := ownerAt(611, 712, 813, 914, 2028)
	same := admission(15, internal, build(28, []byte{
		2,
		3,
	}), build(28, []byte{
		2,
		3,
	}), repeatBefore, repeated)
	differs := admission(15, internal, build(48, []byte{
		2,
		3,
	}), survivorChanged(build(28, []byte{
		2,
		3,
	})), repeatBefore, repeated)
	const (
	...
	for _, input := range []struct {
		...
		retry    daNodeObserverStateAdmission
		followUp string
		want     string
		coupled  string
	}{
		...
		{
			"publish, survivor changed",
			admission(8, nil, build(41, nil, 0x31, 0x32), survivorChanged(minted(build(21, []byte{
				2,
				3,
			}), 1016)), primaryBefore, published, 0x32, 0x31, 0x33),
			same,
		...
		{
			// Refusals with a merely planned victim; this one has an actual candidate release, an unchanged image and a
			// mismatched target commit.
			"refusal, image unchanged",
			admission(8, internal, mismatched(build(21, nil)), mismatched(build(21, nil)), primaryBefore, refused, 0x33),
			differs,
			"REPEAT_IDENTICAL_ADMISSION",
			`{
		...
		{
			// A refusal without a candidate release.
			"refusal, image changed",
			admission(8, internal, mismatched(build(41, nil)), survivorChanged(mismatched(build(21, nil))), primaryBefore, published, 0x33),
			same,
			"REPEAT_IDENTICAL_ADMISSION",
			`{
		"result": ` + internalResult + `,
		...
		t.Run(input.name, func(t *testing.T) {
			err := stateDistinctSources(
				[]uint64{
					input.primary.Invocations,
					input.retry.Invocations,
				},
				chunk,
				[]daNodeObserverStateObservation{
					input.primary.Before,
					input.primary.After,
					input.retry.Before,
					input.retry.After,
				},
				strings.Fields(input.coupled)...,
			)
			require(t, err == nil, "observer state distinct sources %s: %v", input.name, err)
	...
	cleanupImage := func(removed, final bool, base uint64) DAObserverStateImage {
		...
		if removed {
			...
			image.Locators = []DAObserverLocator{
				{TxID: [32]byte{0x65}},
				{TxID: [32]byte{0x67}},
				{
					TxID: [32]byte{0x67},
	...
	for ordinal, txid := range map[uint64]byte{
		0: 0x60,
		1: 0x61,
		5: 0x65,
		6: 0x66,
		...
		cleanupFixture.Members[key] = daNodeObserverStateMember{
			Key:  key,
			DAID: [32]byte{0x70},
			TxID: [32]byte{txid},
		}
	...
	chunkCase := daNodeObserverStateCase{Case: daNodeObserverCase{ID: "STATE_B_PEER_CHUNK_CLEANUP_PRESERVES_NONPEER"}}
	cleanupRow := func(result, selected, released string, locators, claims int, state string, survivors bool, identical string, repeat bool) string {
		return fmt.Sprintf(`{
			"cleanup_result": %q,
		...
		}`, result, selected, released, locators, claims, state, survivors, identical, repeat)
	}
	completeSet := cleanupImage(false, true, 100)
	completeSet.Records[0].State = "COMPLETE_SET"
	for _, input := range []struct {
		name    string
		row     daNodeObserverStateCase
	...
	}{
		{
			...
			[3]DAObserverStateImage{
				cleanupImage(true, true, 300),
				completeSet,
				cleanupImage(false, false, 500),
			},
			cleanupRow("REMOVED_PEER_CHUNKS", "[\n5,\n7\n]", "78", 3, 4, "COMPLETE_SET", true, "", false),
			"StagedBytes",
		},
		{
			"survivor changed",
			protected,
			[3]DAObserverStateImage{
				cleanupImage(false, false, 300),
				cleanupImage(false, true, 100),
				cleanupImage(false, true, 100),
			},
			cleanupRow("NO_SELECTION", "[]", "0", 0, 0, "STAGED_COMMIT", false, `"image_byte_identical": false,`, true),
			"StagedBytes",
		},
		{
			"headers changed",
		...
		},
		{
			...
			cleanupRow("NO_SELECTION", "[]", "0", 0, 0, "STAGED_COMMIT", true, `"image_byte_identical": true,`, true),
			"",
		},
	} {
		t.Run(input.name, func(t *testing.T) {
			first := observe(input.steps[0], 1)
			second := observe(input.steps[1], 106)
			third := observe(input.steps[2], 213)
			err := stateDistinctSources(nil, daNodeObserverStateMember{}, []daNodeObserverStateObservation{
				first,
				second,
				third,
			}, strings.Fields(input.coupled)...)
			require(t, err == nil, "observer state distinct sources %s: %v", input.name, err)
			row, err := projectDANodeObserverStateCleanup(cleanupFixture, input.row, first, second)
			require(t, err == nil, "observer state cleanup: %v", err)
			row["follow_up"], err = stateCleanupFollowUp(cleanupFixture, input.row, second, third)
			require(t, err == nil, "observer state cleanup: %v", err)
			stateAssertJSON(t, row, input.want)
		})
	}
	// A follow-up cleanup that removes members.
	removedFollowUp := []daNodeObserverStateObservation{
		observe(cleanupImage(true, true, 300), 106),
		observe(cleanupImage(false, true, 100), 213),
	}
	require(t, stateDistinctSources(nil, daNodeObserverStateMember{}, removedFollowUp) == nil, "observer state distinct sources: removed follow-up")
	followUp, err := stateCleanupFollowUp(cleanupFixture, protected, removedFollowUp[0], removedFollowUp[1])
	require(t, err == nil, "observer state cleanup: %v", err)
	stateAssertJSON(t, followUp, `{
		"cleanup_result": "REMOVED_PEER_CHUNKS",
		"image_and_claims_unchanged": false,
		"owner_high_water_delta": "107"
	}`)
	// Refusals and removed-member rows that the synthetic rows above do not reach.
	removedBefore := observe(cleanupImage(true, true, 300), 1)
	kept := observe(cleanupImage(false, true, 100), 106)
	stale := kept
	...
	require(t, err != nil && strings.Contains(err.Error(), "retained record is absent"), "observer state cleanup: absent record projected as zero values: %v", err)
	extra := observe(cleanupImage(false, true, 100), 106)
	...
	for _, rows := range [][]DAObserverOutpointRow{
		...
		{{
			Outpoint: consensus.Outpoint{Vout: 99},
			TxID:     owned.TxID,
			TokenSeq: 3000,
		}},
		{owned, owned},
