...
func TestValidateTxLocal_CoreSimplicitySpendRejected(t *testing.T) {
	...
	result := ValidateTxLocal(tvc, [32]byte{}, 1, 0, nil)
	assertTxErrCodeMsg(t, result.Err, TX_ERR_COVENANT_TYPE_INVALID, "CORE_SIMPLICITY deployment not active")
}

// 0x0102 is unassigned per CANONICAL §14 — the worker/queued spend path
// (ValidateTxLocal -> validateTxLocalInputs -> assignedWorkerWitness/WitnessSlots) must
// reject it as TX_ERR_COVENANT_TYPE_INVALID (RUB-585) with opaque covenant_data 070000.
func TestValidateTxLocal_UnknownCovenant0x0102Rejected(t *testing.T) {
	var prevTxid [32]byte
	prevTxid[0] = 0x67
	tx := &Tx{
		Version: 1,
	...
	tvc := TxValidationContext{
		...
		ResolvedInputs: []UtxoEntry{{
			Value:        100,
			CovenantType: 0x0102,
			CovenantData: []byte{0x07, 0x00, 0x00},
		}},
		WitnessStart: 0,
		WitnessEnd:   1,
		Fee:          Uint128FromU64(10),
...
func TestValidateTxLocal_CoreSimplicityInputGroupCapDeferredBehindDisabledSpend(t *testing.T) {
	...
	assertTxErrCodeMsg(t, run(SIMPLICITY_MAX_GROUP_INPUTS, false).Err, TX_ERR_COVENANT_TYPE_INVALID, "CORE_SIMPLICITY deployment not active")
	assertTxErrCodeMsg(t, run(SIMPLICITY_MAX_GROUP_INPUTS+1, true).Err, TX_ERR_COVENANT_TYPE_INVALID, "CORE_SIMPLICITY deployment not active")
