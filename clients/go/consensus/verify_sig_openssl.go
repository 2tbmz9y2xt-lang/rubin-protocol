...
func opensslVerifySig(alg string, pubkey []byte, signature []byte, msg []byte) (bool, error) {
	if alg == "" {
...
func verifySig(suiteID uint8, pubkey []byte, signature []byte, digest32 [32]byte) (bool, error) {
	...
	return opensslVerifySig(alg, pubkey, signature, digest32[:])
}
