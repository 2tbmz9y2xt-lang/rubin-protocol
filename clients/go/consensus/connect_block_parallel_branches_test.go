...
func TestApplyNonCoinbaseTxBasicWorkQ_VaultSpendOK(t *testing.T) {
	...
	q := NewSigCheckQueue(1)
	nextUtxos, fee, err := applyNonCoinbaseTxBasicWorkQ(tx, txid, utxos, 200, 0, [32]byte{}, nil, q)
	if err != nil {
...
func TestApplyNonCoinbaseTxBasicWorkQ_VaultErrorPaths(t *testing.T) {
	...
	t.Run("vault_output_in_spend", func(t *testing.T) {
		...
		q := NewSigCheckQueue(1)
		_, _, err := applyNonCoinbaseTxBasicWorkQ(tx, [32]byte{}, utxos, 200, 0, [32]byte{}, nil, q)
		if err == nil {
...
func TestApplyNonCoinbaseTxBasicWorkQ_CoreExtBranches(t *testing.T) {
	t.Run("inactive_profile_break", func(t *testing.T) {
		...
		q := NewSigCheckQueue(1)
		_, fee, err := applyNonCoinbaseTxBasicWorkQ(tx, hashWithPrefix(0xA1), utxos, 1, 0, [32]byte{}, nil, q)
		if err != nil {
	...
	t.Run("active_profile_mldsa87_queued", func(t *testing.T) {
		...
		q := NewSigCheckQueue(1)
		_, fee, err := applyNonCoinbaseTxBasicWorkQ(tx, hashWithPrefix(0xB1), utxos, 1, 0, [32]byte{}, profiles, q)
		if err != nil {
	...
	t.Run("active_profile_suite_disallowed", func(t *testing.T) {
		...
		q := NewSigCheckQueue(1)
		_, _, err := applyNonCoinbaseTxBasicWorkQ(tx, [32]byte{}, utxos, 1, 0, [32]byte{}, profiles, q)
		if err == nil {
	...
	t.Run("active_profile_ext_verifier_ok", func(t *testing.T) {
		...
		q := NewSigCheckQueue(1)
		_, fee, err := applyNonCoinbaseTxBasicWorkQ(tx, hashWithPrefix(0xD1), utxos, 1, 0, [32]byte{}, profiles, q)
		if err != nil {
...
func TestApplyNonCoinbaseTxBasicWorkQ_AnchorOutputSkip(t *testing.T) {
	...
	q := NewSigCheckQueue(1)
	nextUtxos, fee, err := applyNonCoinbaseTxBasicWorkQ(tx, txid, utxos, 1, 0, [32]byte{}, nil, q)
	if err != nil {
