...
func validateHTLCSpendCompat(
	...
	return ValidateHTLCSpend(entry, pathItem, sigItem, tx, inputIndex, inputValue, chainID, blockHeight, blockMTP)
}

func htlcTestDigest(t *testing.T, sighashType uint8) [32]byte {
	t.Helper()
	tx, inputIndex, inputValue, chainID := testSighashContextTx()
...
func TestValidateHTLCSpend_RefundTimestampUsesMTP(t *testing.T) {
	...
	digest := htlcTestDigest(t, SIGHASH_ALL)
...
var htlcRefundTimelockFirstErrorOrderCases = []htlcRefundTimelockFirstErrorOrderCase{
	...
	{name: "height_immature_only", lockMode: LOCK_MODE_HEIGHT, lockValue: 10, height: 9, payload: []byte{0x01}, selectorMatches: true, want: TX_ERR_TIMELOCK_NOT_MET},
	{name: "timestamp_immature_only", lockMode: LOCK_MODE_TIMESTAMP, lockValue: 20, height: 10, blockMTP: 19, payload: []byte{0x01}, selectorMatches: true, want: TX_ERR_TIMELOCK_NOT_MET},
	...
	{name: "malformed_common_selector_precedes_height_timelock_and_selector_mismatch", lockMode: LOCK_MODE_HEIGHT, lockValue: 10, height: 9, payload: nil, want: TX_ERR_PARSE},
...
func TestValidateHTLCSpend_SigKeyBindingMismatch(t *testing.T) {
	...
	sig.Signature[len(sig.Signature)-1] = SIGHASH_ALL

	err := validateHTLCSpendCompat(entry, path, sig, digest, 0, 0)
	if err == nil {
		t.Fatalf("expected error")
	}
	if got := mustTxErrCode(t, err); got != TX_ERR_SIG_INVALID {
...
func TestValidateHTLCSpend_SignatureInvalid(t *testing.T) {
	...
	sig := WitnessItem{
		SuiteID:   SUITE_ID_ML_DSA_87,
		Pubkey:    claimPub,
		Signature: make([]byte, ML_DSA_87_SIG_BYTES+1), // wrong signature but correct length
	}
	sig.Signature[len(sig.Signature)-1] = SIGHASH_ALL
...
func TestValidateHTLCSpend_ClaimOK(t *testing.T) {
	...
	if err := validateHTLCSpendCompat(entry, path, sig, digest, 0, 0); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
