...
func timelockData(mode byte, lockValue uint64) []byte {
	b := make([]byte, MAX_TIMELOCK_COVENANT_DATA)
	b[0] = mode
	binary.LittleEndian.PutUint64(b[1:], lockValue)
	return b
}
...
func TestApplyNonCoinbaseTxBasic_TimelockHeightNotMet(t *testing.T) {
	var prev [32]byte
	prev[0] = 0xac
	txBytes := txWithOneInputOneOutput(prev, 0, 1, COV_TYPE_P2PK, validP2PKCovenantData())
	tx, txid := mustParseTxForUtxo(t, txBytes)
	...
	utxos := map[Outpoint]UtxoEntry{
		{Txid: prev, Vout: 0}: {
			...
			CovenantData: timelockData(0x00, 200),
		},
	}
	_, err := ApplyNonCoinbaseTxBasic(tx, txid, utxos, 100, 1000)
	if err == nil {
...
func TestApplyNonCoinbaseTxBasic_TimelockTimestampNotMet(t *testing.T) {
	var prev [32]byte
	prev[0] = 0xad
	txBytes := txWithOneInputOneOutput(prev, 0, 1, COV_TYPE_P2PK, validP2PKCovenantData())
	tx, txid := mustParseTxForUtxo(t, txBytes)
	...
	utxos := map[Outpoint]UtxoEntry{
		{Txid: prev, Vout: 0}: {
			Value:        1,
			CovenantType: COV_TYPE_TIMELOCK,
			CovenantData: timelockData(0x01, 1500),
		},
	}
