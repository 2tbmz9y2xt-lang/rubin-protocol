...
For reads/writes, the stall interval is `min(configured positive timeout, 15_000 ms)`. Its deadline starts at `frame_start`, refreshes only on positive bytes (including bytes returned with an error), and is unchanged by zero-byte errors/retries. The effective deadline is `min(stall_deadline, absolute_deadline)`. A `blocktxn` retains that inherited first-byte effective deadline. An interrupted compact fallback may clear its outstanding request and record late context only after its complete header and payload write succeeds within that same deadline. A write clock starts before its first header-byte attempt, after payload validation. Equality is accepted; later failures never reset/extend timing. Handshake timing is unchanged.
