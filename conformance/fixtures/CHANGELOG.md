...
The BUILDING structural gate now mirrors six checks on both sides: full included-set commit/chunk identity grammar, flat occurrence multiplicity before normalization, exact OWNER_TOKEN claim-to-retained txid binding, consecutive encoded summary heights, scalar u64 count aliases, and a closed Go row/case key envelope that admits schema-legal row `notes`/`authority`. Named-case truth, effects, prestates, consumers and runtime conformance remain RUB-1204 activation work.
