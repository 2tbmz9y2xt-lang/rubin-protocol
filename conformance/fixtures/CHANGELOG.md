...
New generator-owned artifact `protocol/canonical_pipeline_v2.json` with the versioned schema `conformance/schemas/cv-canonical-pipeline-v2.json` (schema version 2), registered as **BUILDING** — not an authority. `protocol/canonical_pipeline_v1.json` and `conformance/schemas/cv-canonical-pipeline-v1.json` are unchanged byte-frozen read-only parents (their exact sha256 pins live in the v2 `_meta` and are recomputed by the generator tests) and stay the inert authority until RUB-1204 activates v2 and deletes them in one PR.

The revision carries identity and shape only: `_meta.closure_epoch` binds it to the RUB-1206 design-closure manifest `rubin-c01-design-closure-v3` (every hash a generator constant and a schema `const`, so a one-sided edit of either copy fails generation or validation; the binding to the snapshot itself is by pinned hash verified at claim, not re-verified in-tree); `row_registry` is the frozen `row_id -> kind` map of the 79 C01-R2 identities — the 62 inherited from v1 plus the 17 the closure epoch authorizes — with migration status derived, never stored (a row is migrated exactly when its id appears in `rows`); `rows` and the deterministic alias catalog `fixtures` are empty. The schema closes the machine input/case normal form (typed `input` with a production setup sink per pointer, typed `expected` with the nullable result plus `pipeline_reached`, the closed `wire_disposition`/`recovery_outcome`/RPC class domains, per-image projection and the `null`/`[]`/rows summary rule) and the generator pins the spec 6.4.1 result → commit-truth relation and the typed RPC domains. `detail`, `scenario` and `pending_owner` are not part of the v2 shape; the static per-row `release_requirements` replaces `pending_owner` and states a requirement, never current readiness. No consumer may claim or bind the revision while its closure-epoch status is `building`.
