...
| corpus closure: every `fixtures` entry is named by some case (the catalog twin of the `resolved_values` reachability gate), every stated `/input/...` pointer is in the closed vocabulary, no stated array repeats an alias, and no identity list repeats an identity | `cp2ValidateR1208Payload` / `validate_canonical_pipeline_v2_semantics` | `orphan fixtures entry`, `input pointer typo`, `stated array repeats an alias` |
